...
S3_LEGACY=""
S3_EOS=""
S3_BUCKET="s3://releases.dataorb.co"
...
then
  ...
  S3_STABLE="${S3_BUCKET}/${BRANCH}/dataorb-stable-${PATCH}.war"
  S3_LEGACY="${S3_BUCKET}/${BRANCH}/${PATCH}/dlms.war"
else
  S3_DATE=`date -I'date'`
  if [[ "$BRANCH" == "master" ]]; then
    S3_LEGACY="${S3_BUCKET}/dev/dlms.war"
  else
    S3_LEGACY="${S3_BUCKET}/${BRANCH}/dlms.war"
  fi
  S3_CANARY="${S3_BUCKET}/${BRANCH}/canary/dataorb-canary-${BRANCH}.war"
  S3_CANARY_DATE="${S3_BUCKET}/${BRANCH}/canary/dataorb-canary-${BRANCH}-${S3_DATE}.war"
  S3_DEV="${S3_BUCKET}/${BRANCH}/dev/dataorb-dev-${BRANCH}.war"

  for V in "2.31" "2.32" "2.33"
  do	
    if [[ "$BRANCH" == "$V" ]]; then
      # create the "eos" copy
      S3_EOS="${S3_BUCKET}/${BRANCH}/dataorb-stable-${BRANCH}-eos.war"
    fi
  done
fi
