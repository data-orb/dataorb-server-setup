...
VERSIONS_JSON="https://releases.dataorb.co/v1/versions/stable.json"

BUCKET="s3://releases.dataorb.co"
...
WAR_LOCATION="${WORKSPACE}/dataorb/dlms-web/dlms-web-portal/target/dlms.war"
...
case $RELEASE_TYPE in
  "canary")
    S3_DESTINATIONS+=("$BUCKET/$RELEASE_VERSION/$RELEASE_TYPE/dataorb-$RELEASE_TYPE-$RELEASE_VERSION.war")

    S3_DESTINATIONS+=("$BUCKET/$RELEASE_VERSION/$RELEASE_TYPE/dataorb-$RELEASE_TYPE-$RELEASE_VERSION-$BUILD_DATE.war")
    ;;
  ...
  "dev")
    ...
    S3_DESTINATIONS+=("$BUCKET/$RELEASE_VERSION/dlms.war")
    ;;

  "eos")
    S3_DESTINATIONS+=("$BUCKET/$RELEASE_VERSION/dataorb-stable-$RELEASE_VERSION-$RELEASE_TYPE.war")
    ;;
  ...
  "stable")
    ...
    if [[ "$IS_PATCH_VERSION" -eq 1 ]]; then
      ...
      S3_DESTINATIONS+=("$BUCKET/$SHORT_VERSION/dataorb-$RELEASE_TYPE-$RELEASE_VERSION-${GIT_COMMIT:0:7}.war")
    fi

    S3_DESTINATIONS+=("$BUCKET/$SHORT_VERSION/dataorb-$RELEASE_TYPE-$RELEASE_VERSION.war")
    S3_DESTINATIONS+=("$BUCKET/$SHORT_VERSION/$RELEASE_VERSION/dlms.war")
    ...
    if [[ "$IS_PATCH_VERSION" -eq 0 && -n "${LATEST_PATCH_VERSION-}" && "$PATCH_VERSION" -ge "$LATEST_PATCH_VERSION" ]]; then
      S3_DESTINATIONS+=("$BUCKET/$SHORT_VERSION/dataorb-$RELEASE_TYPE-latest.war")
    fi
    ...
    if [[ "$IS_PATCH_VERSION" -eq 1 ]]; then
      ...
      # for release candidates, also add a version with the git hash
      S3_DESTINATIONS+=("$BUCKET/$NEW_SHORT_VERSION/dataorb-$RELEASE_TYPE-$NEW_RELEASE_VERSION-${GIT_COMMIT:0:7}.war")
    fi

    S3_DESTINATIONS+=("$BUCKET/$NEW_SHORT_VERSION/dataorb-$RELEASE_TYPE-$NEW_RELEASE_VERSION.war")
